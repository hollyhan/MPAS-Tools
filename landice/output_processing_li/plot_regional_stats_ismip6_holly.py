...
for reg in range(nRegions):
   ...
   plt.grid()
   axs1.flatten()[reg].set_title(f"{reg+1}. {rNames[reg]}")
   if reg == 0:
...
def plotStat(fname, sty, label, color, addToLegend=False):
 ...
 #      axs1.flatten()[r].plot(yr, vol[:,r] - vol[0,r], label=lbl, linestyle=sty, color='k')
    ...
    if options.units == "m3":
       ...
       VAF = VAF * rhoi / 1.0e12
   # label ='VAF' if addToLegend else lbl
    for r in range(nRegions):
       axs1.flatten()[r].plot(yr, VAF[:,r] - VAF[0,r], label=label, linestyle=sty, color=color)
...
plotStat(options.file1inName, sty='-', label="Control, coupled", color="k")


if(options.file2inName):
    plotStat(options.file2inName, sty='--', label="Control, uncoupled", color="k")

if(options.file3inName):
    plotStat(options.file3inName, sty='-', label="NorESM-RCP26-repeat, coupled", color="b")

if(options.file4inName):
    plotStat(options.file4inName, sty='--', label="NorESM-RCP26-repeat, uncoupled", color="b")

if(options.file5inName):
    plotStat(options.file5inName, sty='-', label="UKESM-SSP585, coupled", color="m")

if(options.file6inName):
    plotStat(options.file6inName, sty='--', label="UKESM-SSP585, uncoupled", color="m")
